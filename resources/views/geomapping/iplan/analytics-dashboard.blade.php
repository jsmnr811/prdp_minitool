                ...
                <div class="card-header bg-white border-bottom p-3">
                    <form method="GET" action="{{ route('geomapping.iplan.investment.analytics-dashboard') }}"
                        ...
                        <div class="d-flex gap-2 flex-wrap">
                            ...
                            <button id="exportExcel" class="btn btn-success btn-sm d-flex align-items-center">
        ...
        <div class="col-12">
            <div class="row g-4">
                                    ...
                                    <thead class="sticky-top bg-white shadow-sm">
                                        ...
                                        <tr class="fw-bold bg-light">
                                            ...
                                            <td class="text-end text-primary">
                                                {{ number_format($totalFundRequirement, 2) }}</td>
                                    ...
                                    <tbody>
                                        @foreach ($provinces as $province)
                                            ...
                                            @forelse ($province->pcipMatrices as $mIndex => $matrix)
                                                <tr>
                                                    @if ($mIndex === 0)
                                                        <td rowspan="{{ $matrixCount ?: 1 }}"
                                                            class="align-middle fw-semibold text-primary bg-light">
                                                            {{ $province->name }}</td>
                                                    @endif
                                                    <td>{{ $matrix->commodity->name ?? '—' }}</td>
                                                    <td>{{ $matrix->intervention->name ?? '—' }}</td>
                                                    <td class="text-end">
                                                        {{ number_format($matrix->funding_requirement, 2) }}</td>
                                                    <td class="text-end">
                                                        @if ($matrix->funded > 0)
                                                            <span
                                                            class="badge bg-success">{{ number_format($matrix->funded, 2) }}</span>@else<span
                                                    ...
                                                    <td class="text-end">
                                                        @if ($matrix->unfunded > 0)
                                                            <span
                                                            class="badge bg-danger">{{ number_format($matrix->unfunded, 2) }}</span>@else<span
                                                                class="text-muted">0.00</span>
                                                        @endif
                                                    </td>
                                                </tr>
                                            @empty
                                                <tr>
                                                    <td class="align-middle fw-semibold text-primary bg-light">
                                                        {{ $province->name }}</td>
                                                    <td colspan="5" class="text-center text-muted">No interventions
                                                    </td>
                                                </tr>
                                            @endforelse
                                        @endforeach
                ...
                <div class="col-lg-3 col-12">
                    <div class="card shadow-sm rounded-3 mb-3 p-2">
                        <div class="text-center fw-bold mb-1">Funded</div>
                        <div id="map-funded" style="height:250px; border-radius:0.5rem;"></div>
                    </div>

                    <div class="card shadow-sm rounded-3 p-2">
                        <div class="text-center fw-bold mb-1">Unfunded</div>
                        <div id="map-unfunded" style="height:250px; border-radius:0.5rem;"></div>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    function addMarkersAndFitBounds(mapId, geoData, type) {
        ...
        geoData.forEach(item => {
            ...
            const commodityIcon = L.icon({ iconUrl, iconSize:[32,32], iconAnchor:[16,32], popupAnchor:[0,-32] });
            const marker = L.marker([parseFloat(item.latitude), parseFloat(item.longitude)], { icon: commodityIcon })
                .addTo(map)
                .bindPopup(`<b>${item.commodity}</b> (${item.province})<br>Intervention: ${item.intervention}<br>${type}: ${type==='Funded'?item.funded:item.unfunded}`);
            markers.push(marker);
        });

        if (markers.length > 0) {
            const group = L.featureGroup(markers);
            map.fitBounds(group.getBounds().pad(0.2)); // pad to add margin around markers
        } else {
            map.setView([12.8797, 121.7740], 5); // default if no markers
        }

        return map;
    }

    const mapFunded = addMarkersAndFitBounds('map-funded', {!! $geoCommoditiesFundedJson !!}, 'Funded');
    const mapUnfunded = addMarkersAndFitBounds('map-unfunded', {!! $geoCommoditiesUnfundedJson !!}, 'Unfunded');
</script>
@endpush
