...
<div style="width:{{ $baseWidth * $scale }}px; height:{{ $baseHeight * $scale }}px; position:relative; overflow:hidden; background:#fff; border-radius:{{ 10 * $scale }}px; box-shadow:0 0 {{ 20 * $scale }}px rgba(0,0,0,0.2); font-family:Arial, sans-serif;">

    <!-- Background -->
    <img src="{{ $bgSrc }}" alt="ID Background" style="position:absolute; top:0; left:0; width:100%; height:100%; z-index:0; object-fit:cover;">

    <!-- Main Content -->
    <div style="position:relative; z-index:1; display:flex; flex-direction:column; align-items:center; justify-content:flex-start; padding:{{ $padding }}px;">

        <!-- Profile Image -->
        <div style="margin-top:{{ $marginTopProfile }}px;">
            <img src="{{ $userImageSrc }}" style="width:{{ $profileImgSize }}px; height:{{ $profileImgSize }}px; object-fit:cover; border:{{ $profileImgBorder }}px solid rgba(255,255,255,0.8); border-radius:{{ $profileImgSize/2 }}px;">
        </div>

        <!-- Names -->
        <div style="display:flex; flex-direction:column; align-items:center; margin-top:{{ $marginTopNames }}px; text-align:center;">
            ...
            <div style="font-family:Arial, sans-serif; font-size:{{ $lastFontSize }}px; font-style:italic; font-weight:600; color:#2f4482; line-height:1.1;">
                {{ $lastName }}
            </div>
        </div>
        ...
        <div style="margin-top:{{ 20 * $scale }}px; text-align:center; width:100%;">
            ...
            <div style="font-size:{{ 8 * $scale }}px; font-style:italic; color:#666; margin-top:{{ $scale }}px; line-height:1;">
                {{
    ...
    <!-- QR & ID at bottom -->
    <div style="position:absolute; bottom:{{ $bottomSection }}px; left:50%; transform:translateX(-50%); display:flex; align-items:center; gap:{{ 20 * $scale }}px; z-index:2;">
        ...
        <div style="text-align:left; line-height:1.2;">
            ...
            <div style="font-size:{{ $idFontSize }}px; color:#2f4482; font-weight:800;">{{ $user->login_code }}</div>
        </div>
